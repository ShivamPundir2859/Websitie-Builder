...
interface SidebarProps {
   ...
   isGenrating: boolean;
   SetIsGenrating: (isGenrating: boolean) => void;
}

const Sidebar = ({ isMenuOpen, project, setProject, isGenrating, setIsGenrating }: SidebarProps) => {
   ...
   useEffect(()=>{
      if(messagRef.current){
         messagRef.current.scrollIntoView({ behavior:"smooth"})
      }
   },[project.conversation.length, isGenrating])
         ...
         <div className='flex flex-col h-full'>
                           ...
                           bg-gray-800 text-gray-100 shadow flex flex-col gap-2'>
                              ...
                              <div>
                                 {project.current_version_index === ver.id ? (
                                    ...
                                    <button onClick={()=> handleRollback(ver.id)} className='px-3 py-1 rounded-md text-xs bg-gray-500
                                    hover:bg-indigo-600 text-white'>Roll back to this Version</button>
                                 )}
                                 <Link target='_blank' to={`/preview/${project.id}/${ver.id}`}>
                                    <EyeIcon className='size-6 p-1 bg-gray-700 hover:bg-gray-500 
            ...
            <form onSubmit={handleRevisions} className='m-3 relative'>
               <div className='flex items-center gap-2'>
                  ...
                  className='flex-1 p-3 rounded-xl resize-none text-sm outline-none ring ring-gray-700 focus:ring-indigo-500 bg-gray-800
                  text-gray-100 placeholder-gray-400 transition-all' disabled={isGenrating}/>
                  <button disabled={isGenrating || !input.trim()} className='absolute bottom-2.5 right-2.5 rounded-full bg-linear-to-r
